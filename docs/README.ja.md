...
- Python 3.10 以降。これより古いインタプリタでは、起動時に警告を出したうえで実行を継続します（コードは 3.10 専用の構文を使っていないため）。ただし、より古いバージョンで発生した予期しない失敗はサポート対象外です。
- npm コマンドラッパーを使う場合にのみ Node.js 22 以降
- Claude Code は任意です。実行時の `/resume` または `/context` のスモークテストを明示的に依頼する場合にのみ必要です。
- 対象ファイルが置かれているボリュームがハードリンクに対応していること（`--replace-original` の場合**のみ**）

Python パッケージのインストールは不要です。

### 稼働中ファイルの置き換えにおけるハードリンク要件

`--replace-original` は、並行して作成された別のファイルを決して上書きしないよう、`os.link` を使って候補ファイルを公開します。ロールバック経路も同じ方法で、取得しておいた元のファイルを復元します。したがってどちらも、セッションファイルが置かれているボリュームがハードリンクに対応している必要があります。

圧縮側は、ステージング・バックアップ・移動のいずれを行う**前に**これを検査します。ファイルシステムが `os.link` を拒否した場合、対象ファイルがそのまま残った状態で、何も書き込まずに実行を停止します。この要件を満たせないことが多いファイルシステムは、FAT32/exFAT のリムーバブルメディア、一部の SMB/NFS マウント、一部のコンテナのバインドマウントです。NTFS と ext4 は問題ありません。

候補ファイルの出力は影響を受けません。こちらは `os.replace` で公開されるため、ハードリンクへの依存はありません。
